...
//    }
  ...
  @IBAction func presentSettings(_ sender: Any? = nil) {
    ...
    settingsController.didDismiss = {
      [unowned self] in self.toggleBlur(false)
    }
...
//                        animations:blurAnimations(blurred)).startAnimation()
  ...
  override func touchesBegan(_ touches: Set<UITouch>,
    ...
    // check if the touch happened during a non-interactive transition and the transition’s animator is currently running. In that case, you store the current touch location and then call your custom method interruptTransition(). This will pause the transition and leave it in an interactive mode.
  ...
  override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
    ...
    if currentPoint < startY - 40 {
      ...
      presentTransition.animator?.addCompletion {
        _ in self.blurView.effect = nil
      }
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    if indexPath.row == 1 {
      ...
      cell.didPressEdit = {[unowned self] in
        ...
        // ensures that you are presenting the settings view controller non-interactively.
